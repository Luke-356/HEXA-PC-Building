...
if (isset($_POST['btn-add'])) {
    if (isset($_SESSION["add_storage"])) {
        $product_array_id = array_column($_SESSION["add_storage"], "storage_id");
        if (!in_array($_GET["id"], $product_array_id)) {
            unset($_SESSION["add_storage"]);
            $product_array = array(
                'storage_id'               =>     $_GET["id"],
                'storage_name'               =>     $_GET["name"],
                'storage_image'               =>     $_GET["image"],
                'storage_price'          =>     $_GET["price"]
            );
            $_SESSION["add_storage"][0] = $product_array;
            echo '<script>window.location.assign("builder.php");</script>';
        } else {
            echo '<script>alert("This storage is already added")</script>';
        }
    } else {
        $product_array = array(
            'storage_id'               =>     $_GET["id"],
        ...
        $_SESSION["add_storage"][0] = $product_array;
        $_SESSION["storage_id"] = $_GET["id"];
        $_SESSION["storage_price"] = $_GET["price"];
        echo '<script>window.location.assign("builder.php");</script>';
    }
}
?>
...
<body onunload="func()">
    <div class="container">
        ...
        <?php
        $select = "SELECT * FROM product WHERE ProductType='Storage' ORDER BY ProductID Desc";
        ...
        ?>
            ...
            ?>
                <form method='POST' action="addStorage.php?id=<?php echo $data['ProductID'] ?> & name=<?php echo $data['ProductName'] ?> & image=<?php echo $data['Image'] ?> & price=<?php echo $data['Price'] ?>">
